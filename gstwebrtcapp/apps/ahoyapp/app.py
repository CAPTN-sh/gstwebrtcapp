...
from gstwebrtcapp.utils.gst import DEFAULT_GCC_SETTINGS, get_app_transceiver_properties, dump_to_dot
...
class AhoyApp(GstWebRTCApp):
    ...
    def __init__(
        self,
        config: GstWebRTCAppConfig = GstWebRTCAppConfig(pipeline_str=BIN_H264_IN_H264_OUT_PIPELINE),
    ):
        self.pipeline = None
        self.webrtcbin = None
        self.webrtcbin_elements = []
        self.source = None
        self.raw_caps = None
    ...
    def _init_pipeline(self) -> None:
        ...
        if self.is_graph:
            dump_to_dot(self.pipeline, Gst.DebugGraphDetails.ALL, "ahoyapp")
        ...
        # get scaling elements to control letterboxing
        self.scaling_elements = self.get_scaling_elements()
    ...
    def get_raw_caps(self) -> Gst.Caps:
        s = f"video/x-raw,width={self.resolution['width']},height={self.resolution['height']},framerate={self.framerate}/1,"
        return Gst.Caps.from_string(s)

    def get_scaling_elements(self) -> List[Gst.Element]:
        vs = self.pipeline.get_by_name("videoscale")
        vcs = self.pipeline.get_by_name("videoconvertscale")
        cs = self.pipeline.get_by_name("cudascale")
        ccs = self.pipeline.get_by_name("cudaconvertscale")
        return [e for e in [vs, vcs, cs, ccs] if e]
    ...
    def _cb_deep_element_added(self, _, __, element) -> None:
        if element:
            self.webrtcbin_elements.append(element)
