...
class MDP(metaclass=ABCMeta):
    ...
    def check_observation(self, obs: Dict[str, Any]) -> bool:
        ...
        rtp_inbounds = find_stat(obs, GstWebRTCStatsType.RTP_REMOTE_INBOUND_STREAM)
        if not rtp_inbounds:
            return False
        ...
        if self.obs_filter is not None:
            for stat in self.obs_filter:
                if stat != GstWebRTCStatsType.RTP_REMOTE_INBOUND_STREAM:
...
class ViewerMDP(MDP):
    ...
    def make_state(self, stats: Dict[str, Any], action: Any) -> OrderedDict[str, Any]:
        ...
        rtp_inbound = find_stat(stats, GstWebRTCStatsType.RTP_REMOTE_INBOUND_STREAM)
...
class ViewerSeqMDP(MDP):
    ...
    def make_state(self, stats: Dict[str, Any], action: Any) -> OrderedDict[str, Any]:
        ...
        last_rtp_inbound = (
            find_stat(stats, GstWebRTCStatsType.RTP_REMOTE_INBOUND_STREAM) if self.last_stats is not None else None
        )
        self.last_stats = stats
